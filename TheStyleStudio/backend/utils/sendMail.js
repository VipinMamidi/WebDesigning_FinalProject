...
const sendMail = async (id, email, option) => {
  ...
  if (option === "email verification") {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Confirm your email for The Style Studio", // Subject line
      html: `<div>
  ...
  // send a mail for resetting password if forgot password
    ...
    const mailOptions = {
      from: process.env.EMAIL, // sender address
      to: email,
      subject: "Reset Password for The Style Studio", // Subject line
      html: `<div>
					<h2>Reset Password for your The Style Studio account</h2>
					<br/>
